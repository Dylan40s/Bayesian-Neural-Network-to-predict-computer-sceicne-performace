...
#varaibles that control the number of input and output
...
# 5 layer network, 7 input nodes -> h1 -> h2 -> h3 -> 2 output nodes
def neural_network(X,W_0,W_1,W_2,W_3,b_0,b_1,b_2,b_3,N):
	h1= tf.nn.relu(tf.matmul(X, W_0) + b_0)
	h2= tf.nn.relu(tf.matmul(h1, W_1) + b_1)
	h3= tf.nn.relu(tf.matmul(h2, W_2) + b_2)
	output = tf.nn.sigmoid(tf.matmul(h3, W_3) + b_3)
...
def loadParams():
	...
	for i in range(len(hidden)):
		for h2 in range(len(hidden)):
			for h3 in range(len(hidden)):
				for para1 in range(len(params)):
					for para2 in range(len(params)):
							for ite in range(len(iter1)):
								name = str(hidden[i]) + "H"+str(hidden[h2]) + "H"+str(hidden[h3]) + "H" + str(params[para1]) +"P1"+ str(params[para2]) + "P2" + str(params[para2]) + "P3" + str(params[para1]) +"P4"+ str(iter1[ite])
								returned.append([name, hidden[i],hidden[h2],hidden[h3],params[para1],params[para2], iter1[ite]])
	return returned

# Used to train, takes in featrues, updated marks and file to save to
# returns the amount of correct predictinons which can be used for model comparison
def save(arr,xdata,ydata):
	...
	with graph1.as_default():
		with tf.name_scope("model"):
			...
			W_2 = Normal(loc=tf.zeros([H2, H3]), scale=param2*tf.ones([H2, H3]), name="W_2")
			W_3 = Normal(loc=tf.zeros([H3, FLAGS.O]), scale=param1*tf.ones([H3, FLAGS.O]), name="W_3")
			b_0 = Normal(loc=tf.zeros(H1), scale=param1 *tf.ones(H1), name="b_0")
			b_1 = Normal(loc=tf.zeros(H2), scale=param2* tf.ones(H2), name="b_1")
			b_2 = Normal(loc=tf.zeros(H3), scale=param2* tf.ones(H3), name="b_2")
			b_3 = Normal(loc=tf.zeros(FLAGS.O), scale=param1* tf.ones(FLAGS.O), name="b_3")

			X = tf.placeholder(tf.float32, [trainSetNumber, FLAGS.D], name="X")
			y = Normal(loc=neural_network(x_train,W_0, W_1, W_2,W_3, b_0, b_1, b_2,b_3, trainSetNumber), scale=0.1*tf.ones([trainSetNumber,FLAGS.O]), name="y")
		...
		with tf.variable_scope("posterior",reuse=tf.AUTO_REUSE):
			...
			with tf.variable_scope("qW_2",reuse=tf.AUTO_REUSE):
			    loc = tf.get_variable("loc", [H2, H3])
			    scale = param2*tf.nn.softplus(tf.get_variable("scale", [H2, H3]))
			    qW_2 = Normal(loc=loc, scale=scale)
			with tf.variable_scope("qW_3",reuse=tf.AUTO_REUSE):
			    loc = tf.get_variable("loc", [H3, FLAGS.O])
			    scale = param1*tf.nn.softplus(tf.get_variable("scale", [H3, FLAGS.O]))
			...
			with tf.variable_scope("qb_2",reuse=tf.AUTO_REUSE):
			    loc = tf.get_variable("loc", [H3])
			    scale =param2 * tf.nn.softplus(tf.get_variable("scale", [H3]))
			    qb_2 = Normal(loc=loc, scale=scale)
			with tf.variable_scope("qb_3",reuse=tf.AUTO_REUSE):
			    loc = tf.get_variable("loc", [FLAGS.O])
			    scale =param1 * tf.nn.softplus(tf.get_variable("scale", [FLAGS.O]))
			    qb_3 = Normal(loc=loc, scale=scale)
	
	#inference
	with tf.Session(graph=graph1) as sess:
		# Set up the inference method, mapping the prior to the posterior variables
		inference = ed.KLqp({W_0: qW_0, b_0: qb_0,W_1: qW_1, b_1: qb_1,W_2: qW_2, b_2: qb_2,W_3: qW_3, b_3: qb_3}, data={X: x_train, y: y_train})
		...
		infered = neural_network(x_test, qW_0, qW_1, qW_2, qW_3, qb_0, qb_1, qb_2, qb_3, len(x_test))
		...
		NormalTest =Normal(loc=neural_network(x_test, qW_0, qW_1, qW_2, qW_3, qb_0, qb_1, qb_2, qb_3,len(x_test)), scale=0.1*tf.ones([len(x_test),FLAGS.O]), name="y_other") 
		NormalTestList = NormalTest.eval()
		
		# Change the graph so that the posterior point to the output
		y_post = ed.copy(NormalTest, {W_0: qW_0, b_0: qb_0,W_1: qW_1, b_1: qb_1,W_2: qW_2, b_2: qb_2,W_3: qW_3, b_3: qb_3})
		...
		# PPC calculation
		PPCMean = ed.ppc(lambda xs, zs: tf.reduce_mean(xs[y_post]), data={y_post:  y_test, X:x_test}, latent_vars={W_0: qW_0, b_0: qb_0,W_1: qW_1, b_1: qb_1,W_2: qW_2, b_2: qb_2,W_3: qW_3, b_3: qb_3}, n_samples=5)
		# Change the graph again, this is done to do epistemic uncertainty calculations
		posterior = ed.copy(NormalTest, dict_swap={W_0: qW_0.mean(), b_0: qb_0.mean(),W_1: qW_1.mean(), b_1: qb_1.mean(),W_2: qW_2.mean(), b_2: qb_2.mean(),W_3: qW_3.mean(), b_3: qb_3.mean()})
...
def main():
	...
	print(str(current) + " highest correct Predictions out of 100")
